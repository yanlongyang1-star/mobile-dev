...
| Firebase Test Lab | `docs/firebase-test-lab-script.md` gives APK upload and Robo test steps. Campus screen shows Test Lab readiness. | Firebase Test Lab matrix, device, logs and screenshots after APK build. |
...
| Testing: Test Lab + Jest | Jest tests cover date utilities, booking integration and borrower flow. Test Lab script included. | Terminal screenshot of `npm run test`; Firebase Test Lab evidence. |
| APK and Builds | `eas.json` has Android APK preview/development profiles and `npm run build:android:preview`. | EAS build URL or APK artifact screenshot. |
